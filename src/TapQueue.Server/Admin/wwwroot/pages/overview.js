...
export async function render(root, ctx) {
  ...
  // Only what the admin's role lets them see; a part they can't see is left out rather than shown empty.
  const get = (area, path) => (ctx.can(area) ? api.get(path) : null);
  ...
  async function load(refresh) {
    ...
    const outdated = latestBuild && clients ? clients.filter((c) => !published.has(c.clientVersion)) : [];

    body.replaceChildren(
      h("div", { class: "grid stats" },
        held && stat("Held jobs", held.length, "jobs", waiting(held)),
        printers && stat("Printers ready", [ready, h("small", null, ` / ${printers.length}`)], "printers", printerNote(printers, online, ready)),
        stations && stat("Stations online", [stations.filter((s) => s.online).length, h("small", null, ` / ${stations.length}`)], "stations",
          stations.length === 0 ? "None set up" : stations.every((s) => s.online) ? "All reporting in" : `${stations.filter((s) => !s.online).length} offline`),
        clients && stat("Workstations", clients.length, "workstations", "Signed in now"),
      ),
...
/** Parts the admin can't see come in as null and are skipped. */
function attention({ printers, stations, unknown, queues, outdated, latestBuild, server, crashes }) {
  ...
  for (const s of printers ? stations ?? [] : []) {
